...
<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
  <channel>
    <title>BigData Crawler for {$logged_in_user}</title>
    ...
    <description>Calls to this feed will launch the BigData crawler, if it hasn't run in the last {$rss_crawler_refresh_rate} minutes</description> 
    <pubDate>{$smarty.now|date_format:"%a, %d %b %Y %H:%M:%S %Z"}</pubDate>
    <lastBuildDate>{$smarty.now|date_format:"%a, %d %b %Y %H:%M:%S %Z"}</lastBuildDate>
    <generator>BigData v{$thinkup_version}</generator>
