...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='Mentions captured per user in the data store. One row per user. Once the Twitter REST crawler updates the cache_count row, we can move that column into tu_users and drop this table.';
...
CREATE TABLE IF NOT EXISTS tu_mentions_posts (
    ...
    UNIQUE KEY post_id_2 (mention_id, post_id)
  ) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='Mentions captured per post in the data store. Multiple mentions per post.';
...
CREATE TABLE IF NOT EXISTS tu_hashtags (
    ...
    PRIMARY KEY (id),
    unique key hashtag (network, hashtag)
...
CREATE TABLE IF NOT EXISTS tu_hashtags_posts (
    ...
    UNIQUE KEY ht (hashtag_id,post_id)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT 'Hashtags captured per post in the datastore. Multiple hashtags per post.';
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT 'Places posts were published from on a given network. One row per place.';
...
CREATE TABLE IF NOT EXISTS tu_stream_data (
    ...
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT 'Temporary holding area for incoming, unprocessed stream data.';
...
CREATE TABLE IF NOT EXISTS tu_stream_procs (
    ...
    UNIQUE KEY owner (email,instance_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT 'Running stream process details.';
